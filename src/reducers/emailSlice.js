...
const emailSlice = createSlice({
  ...
  reducers: {
    ...
    deleteSentMails(state, action) {
      const updatedMails = state.sentMails.filter((m) => m.id !== action.payload)
      state.sentMails = updatedMails
    },

    deleteReceivedMails(state, action) {
      const updatedMails = state.receivedMails.filter((m) => m.id !== action.payload)
      state.receivedMails = updatedMails
    },
  },
})

export const { setSentMails, setReceivedMails, markAsRead, markAsUnread, deleteSentMails, deleteReceivedMails } =
  emailSlice.actions
